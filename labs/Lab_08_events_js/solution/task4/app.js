...
let colorBlock = document.createElement("div")
  ...
  colorBlock.addEventListener('click', function() {
    // Получаем сохраненный цвет из локального хранилища
     savedColor = localStorage.getItem('backgroundColor');
    
    // Изменяем фоновый цвет блока, исключая главную палитру и палитру с созданными цветами
    if (!colorBlock.classList.contains('colorBlock') && !colorBlock.classList.contains('block')) {
      colorBlock.style.backgroundColor = savedColor;
    }
  });
...
colorBlock.addEventListener('click', paintingBlock);
